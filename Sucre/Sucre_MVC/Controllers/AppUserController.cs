...
namespace Sucre_MVC.Controllers
{
    ...
    public class AppUserController : Controller
    {
        ...
        public async Task<IActionResult> ListAppRoleUsers(Guid? Id)
        {
            ...
            IEnumerable<AppUserTdo> appRoleUsersAssigned = new HashSet<AppUserTdo>();
            IEnumerable<AppUserTdo> appRoleUsersNotAssigned = new HashSet<AppUserTdo>();

            AppRoleTdo appRoleTdo = _roleService.GetAppRoleUsers(Id.Value,
                ref appRoleUsersAssigned,
                ref appRoleUsersNotAssigned);

            if (appRoleTdo == null) return BadRequest($"Role Id={Id.Value} bad or failer!!!"); View();
        ...
        public async Task<IActionResult> UpsertAppUser(Guid? Id)
        {
            ...
            {
                var appUserTdo = await _userService.GetAppUserAsync(Id.Value);

                if (appUserTdo != null)
                {
                    appUserM.Id = appUserTdo.Id;
                    appUserM.Name = appUserTdo.Name;
                    appUserM.Description = appUserTdo.Description;
                    appUserM.Email = appUserTdo.Email;
                    appUserM.PasswordHash = appUserTdo.PasswordHash;
                    appUserM.GroupNumber = appUserTdo.GroupNumber;
                    return View(appUserM);
                }
                else
                {
        ...
        public async Task<IActionResult> UpsertAppUser(AppUserM appUserM)
        {
            ...
            if (ModelState.IsValid)
            {
                AppUserTdo appUserTdo = new AppUserTdo();
                if (appUserM.Id == null || appUserM.Id == Guid.Empty)
                {
                    appUserTdo.Name = appUserM.Name;
                    appUserTdo.Description = appUserM.Description;
                    appUserTdo.Email = appUserM.Email;
                    appUserTdo.PasswordHash = WM.GenerateMD5Hash(
                        appUserM.PasswordHash,
                        _configuration["AppSettings:PasswordSalt"]);
                    appUserTdo.GroupNumber = appUserM.GroupNumber;
                    //await _sucreUnitOfWork.repoSucreCex.AddAsync(cex);
                }
                else
                {
                    //Update
                    appUserTdo.Id = appUserM.Id;
                    appUserTdo.Name = appUserM.Name;
                    appUserTdo.Description = appUserM.Description;
                    appUserTdo.Email = appUserM.Email;
                    appUserTdo.PasswordHash = appUserM.PasswordHash;
                    appUserTdo.GroupNumber = appUserM.GroupNumber;
                }
                bool commitRole = await _userService.UpsertUserAsync(appUserTdo);
                if (commitRole)
        ...
        {            
            ...
            IEnumerable<AppRoleTdo> appUserRolesAssigned = new HashSet<AppRoleTdo>();
            IEnumerable<AppRoleTdo> appUserRolesNotAssigned = new HashSet<AppRoleTdo>();

            AppUserTdo appUserTdo = _userService.GetAppUserRoles(
                Id.Value,
                ref appUserRolesAssigned,
                ref appUserRolesNotAssigned);

            if (appUserTdo == null) return BadRequest($"User Id={Id.Value} bad or failer!!!"); 

            AppUserRolesM appUserRolesM = new AppUserRolesM
            {
                Id = Id.Value,
                Name = appUserTdo.Name,
                Email = appUserTdo.Email,
            };
        ...
        public async Task<IActionResult> ChangeProfileCurUser([FromBody]Dictionary<string,string> obj)
        {
            try
            {
                ...
                AppUserTdo userTdo = await _userService.GetAppUserAsync(email);
                bool update = false;
        ...
        public async Task<IActionResult> ChangePswdCurUser([FromBody] Dictionary<string, string> obj)
        {
            try
            {
                ...
                AppUserTdo userTdo = await _userService.GetAppUserAsync(User.Identity.Name);
