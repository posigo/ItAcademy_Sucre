...
namespace Sucre_Services
{
    public class UserService : IUserService
    {
        ...
        public UserService(
            IConfiguration configuration, ISucreUnitOfWork sucreUnitOfWork)
        {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public async Task<AppUserTdo> GetAppUserAsync(Guid id)
        {
        ...
        /// <returns></returns>
        public async Task<AppUserTdo> GetAppUserAsync(string email)
        {
        ...
        /// <returns></returns>
        public AppUserTdo GetAppUserRoles(
            Guid id,
            ...
            ref IEnumerable<AppRoleTdo> appRolesNotAssigned)
        {
            try
            {
                ...
                {
                    appRolesAssigned = new HashSet<AppRoleTdo>();
                }
                else
                {
                    appRolesAssigned = new HashSet<AppRoleTdo>();
                    appRolesAssigned = appUserDb.AppRoles
                    .Select(role => new AppRoleTdo
                    {
                        Id = role.Id,
                        Name = role.Name,
                        Value = role.Value
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<IEnumerable<AppUserTdo>> GetListUsersAsync()
        {
            try
            {
                ...
                includeProperties: $"{WC.AppRolesName}",
                isTracking: false);
                IEnumerable<AppUserTdo> appUsers = usersDb
        ...
        /// <returns></returns>
        public async Task<int> RegisterUserAsync(AppUserRegTdo userTdo)
        {
        ...
        /// <returns></returns>
        public async Task<bool> UpsertUserAsync(AppUserTdo appUserTdo)
        {
            try
            {
                ...
                AppUser appUser = new AppUser()
                {
                    ...
                    Description= appUserTdo.Description,
                    Email = appUserTdo.Email,
                    PasswordHash= appUserTdo.PasswordHash,
                    
                    //GroupId = 0,
                    //GroupUser = null
                };
                if (appUserTdo.Id == null || appUserTdo.Id == Guid.Empty)
                {
                ...
                {
                    appUser.GroupNumber = appUserTdo.GroupNumber;
